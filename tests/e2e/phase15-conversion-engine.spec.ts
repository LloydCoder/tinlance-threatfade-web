  ...
  test("captures page view with attribution and canonical CTA event", async ({ page }) => {
    ...
    await page.route("**/api/analytics/event", async (route) => {
      if (route.request().method() === "POST") {
        try {
          const body = route.request().postDataJSON() as { name?: string; path?: string; utm_source?: string };
          events.push(body);
        } catch {
    ...
    await expect(page.getByRole("heading", { name: /Find where your existing detection stack/i })).toBeVisible();
    await expect.poll(() => events.some((event) => event.name === "page_view" && event.utm_source === "playwright")).toBe(true);

    await page.locator('[data-tf-event="assessment_request"]').first().dispatchEvent("click");
    await expect.poll(() => events.some((event) => event.name === "assessment_request" && event.path === "/assessment")).toBe(true);
  });

  test("rejects malformed analytics payloads at the server boundary", async ({ request, baseURL }) => {
