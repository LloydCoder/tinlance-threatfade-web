...
test.describe("Phase 16 research scale", () => {
  test("research index exposes search and evidence metadata", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Evidence, methodology and the limits of the evidence." })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: /Behavioral Fade Detection Reproducibility Study/i })).toBeVisible();
  });

  test("challenge page preserves the evidence boundary", async ({ page }) => {
    await page.goto("/research/challenge");
    await expect(page.getByRole("heading", { name: /Can you detect the fade without cheating the evidence/i })).toBeVisible();
