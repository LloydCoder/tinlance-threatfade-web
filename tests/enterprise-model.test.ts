...
import { capabilityCounts, enterpriseCapabilities, enterpriseIntegrationContract } from "@/lib/enterprise/model";
...
describe("enterprise capability model", () => {
  it("keeps enterprise claims explicitly classified", () => {
    ...
    expect(enterpriseCapabilities.every((item) =>
  ...
  it("provides a bounded integration contract", () => {
    ...
    expect(enterpriseIntegrationContract.every((item) => item.status === "contract-defined")).toBe(true);
  });

  it("returns internally consistent status counts", () => {
    const counts = capabilityCounts();
    expect(Object.values(counts).reduce((sum, count) => sum + count, 0)).toBe(enterpriseCapabilities.length);
  });
});
