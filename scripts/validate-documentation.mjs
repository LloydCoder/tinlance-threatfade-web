...
for (const [name, content] of [
  ...
  if (!content.includes(engineUrl)) {
    errors.push(`${name} must identify the engine repository as a source of truth.`);
  }
  if (!content.includes(webUrl)) {
    errors.push(`${name} must identify the web repository boundary.`);
  }
}

if (!readme.includes("evidence-first")) {
...
if (!phase5.includes("current ThreatFade engine repository")) {
  errors.push(
    "docs/PHASE-5-DOCUMENTATION.md must constrain public docs to the current engine repository.",
  );
}
...
if (fs.existsSync(phaseDir)) {
  ...
  for (const name of phaseFiles) {
    ...
    if (!/^#.*phase/i.test(content)) {
      warnings.push(`Phase document ${name} has no obvious phase heading.`);
    }
  }
}
...
if (gate.includes("[ ] Manual assistive-technology verification")) {
  errors.push(
    "Phase 11 gate contains an unresolved accessibility checkbox; represent this as a documented release-owner boundary instead.",
  );
}
if (gate.includes("[ ] Real-user Core Web Vitals")) {
  errors.push(
    "Phase 11 gate contains an unresolved field-performance checkbox; represent this as a documented production-telemetry boundary instead.",
  );
}
