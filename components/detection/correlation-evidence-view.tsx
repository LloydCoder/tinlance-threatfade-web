...
const observations = [
  { domain: "GNSS", label: "signal disruption", offset: 32, width: 16, tone: "warning" as const },
  { domain: "NETWORK", label: "C2 fade", offset: 39, width: 18, tone: "signal" as const },
];
  ...
  return (
    <TfPanel className="overflow-hidden p-6">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        ...
        <TfMetric label="Clock tolerance" value="5s" detail="explicit policy bound" />
        <TfMetric label="Attribution" value="Non-causal" detail="causal attribution not established" />
      </div>
          ...
          {observations.map((item) => (
            <div key={item.domain} className="grid grid-cols-[5.5rem_1fr] items-center gap-4">
              ...
              <div className="relative h-10 rounded-md border border-[var(--tf-line)] bg-[var(--tf-bg)]">
                <div
                  className={`absolute top-1/2 h-6 -translate-y-1/2 rounded-md border px-3 py-1 text-[10px] font-mono ${item.tone === "warning" ? "border-[color-mix(in_srgb,var(--tf-warning)_30%,transparent)] bg-[color-mix(in_srgb,var(--tf-warning)_8%,transparent)] text-[var(--tf-warning)]" : "border-[color-mix(in_srgb,var(--tf-signal)_30%,transparent)] bg-[var(--tf-signal-soft)] text-[var(--tf-signal)]"}`}
