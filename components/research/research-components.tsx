...
export function ResearchToc({ headings }: { headings: { id: string; title: string }[] }) {
  return <nav aria-label="On this page" className="tf-panel p-5"><TfLabel>On this page</TfLabel><ol className="mt-4 space-y-2 text-sm text-[var(--tf-text-muted)]">{headings.map((heading) => <li key={heading.id}><a className="transition hover:text-[var(--tf-signal)]" href={`#${heading.id}`}>{heading.title}</a></li>)}</ol></nav>;
}

export function ResearchCallout({ children, title = "Evidence boundary" }: { children: ReactNode; title?: string }) {
  return <TfPanel raised className="my-8 p-5"><TfLabel>{title}</TfLabel><div className="mt-3 text-sm leading-7 text-[var(--tf-text-muted)]">{children}</div></TfPanel>;
}

export function ResearchCode({ children }: { children: ReactNode }) {
  return <pre className="my-8 overflow-x-auto rounded-lg border border-[var(--tf-line)] bg-[var(--tf-ink)] p-5 font-mono text-xs leading-6 text-[var(--tf-text-muted)]">{children}</pre>;
}
