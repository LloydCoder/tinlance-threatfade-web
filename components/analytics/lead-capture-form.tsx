...
export function LeadCaptureForm({
  ...
  return (
    <div className="mt-8 rounded-2xl border border-[var(--tf-line)] bg-[var(--tf-surface)] p-6 sm:p-8">
      <div className="mb-5">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--tf-text-subtle)]">Technical intake</p>
      ...
      <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
        <label className="text-sm">
          Work email
          <input name="email" type="email" autoComplete="email" required maxLength={254} className="tf-input mt-2 w-full" />
        </label>
        <label className="text-sm">
          Company
          <input name="company" autoComplete="organization" required maxLength={160} className="tf-input mt-2 w-full" />
        </label>
        <label className="text-sm">
          Role
          <input name="role" autoComplete="organization-title" required maxLength={120} className="tf-input mt-2 w-full" />
        </label>
        ...
        <div className="md:col-span-2 flex flex-wrap items-center gap-3">
          <button type="submit" disabled={status === "submitting"} className="tf-button tf-button-primary disabled:opacity-60">
