...
import { ArrowRight, BookOpen, GithubIcon, Layers3, ShieldCheck } from "lucide-react";
...
export function DetectionPipeline() {
  return (
    <div className="grid gap-px overflow-hidden rounded-2xl border border-[var(--tf-line)] bg-[var(--tf-line)] lg:grid-cols-7">
      {steps.map(([number, title, body]) => (
        <article key={number} className="bg-[var(--tf-panel)] p-5 sm:p-6">
...
export function AudienceCards() {
  const audiences = [
    ...
    {
      title: "Researchers",
      label: "Methodology / evidence",
      body: "Follow the detection methodology, deterministic validation and research boundary without confusing project evidence with independent assurance.",
      href: "/research",
      external: false,
      icon: BookOpen,
    },
    {
      title: "SOC teams",
      label: "Detect / investigate / handoff",
      body: "Evaluate evidence-backed detections, analyst workflow, ATT&CK context and operational interoperability.",
      href: "/detection",
      external: false,
      icon: Layers3,
    },
    {
      ...
      body: "Review identity, tenancy, audit, deployment boundaries and the evidence-versus-assurance distinction.",
      href: "/security",
      external: false,
      icon: ShieldCheck,
    },
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {audiences.map(({ title, label, body, href, external, icon: Icon }) => (
        <TfPanel
          key={title}
          className="group p-6 transition hover:border-[color-mix(in_srgb,var(--tf-signal)_30%,var(--tf-line))]"
        >
          <div className="flex items-center justify-between">
            <TfBadge tone="neutral">{label}</TfBadge>
            <Icon className="size-4 text-[var(--tf-text-subtle)] transition group-hover:text-[var(--tf-signal)]" />
          </div>
          ...
          {external ? (
            ...
            >
              Open GitHub <ArrowRight className="size-4" />
            </a>
          ) : (
            <Link
              href={href}
      ...
      <div className="mt-5 grid gap-6 md:grid-cols-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--tf-text-muted)]">
            Tests, benchmarks, controls and documented validation are inspectable in the source
            repository.
          </p>
        </div>
        <div>
          <h3 className="font-semibold">Project validation</h3>
          <p className="mt-2 text-sm leading-6 text-[var(--tf-text-muted)]">
            Published validation is explicitly scoped; it is not presented as a universal accuracy
            guarantee.
          </p>
        </div>
        <div>
          <h3 className="font-semibold">External assurance</h3>
          <p className="mt-2 text-sm leading-6 text-[var(--tf-text-muted)]">
            Certifications, independent testing, contractual SLAs and customer-scale guarantees
