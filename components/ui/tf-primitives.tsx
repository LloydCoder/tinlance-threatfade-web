...
export function TfBadge({
  ...
  const tones = {
    ...
    signal:
      "border-[color-mix(in_srgb,var(--tf-signal)_28%,transparent)] bg-[var(--tf-signal-soft)] text-[var(--tf-signal)]",
    warning:
      "border-[color-mix(in_srgb,var(--tf-warning)_28%,transparent)] bg-[color-mix(in_srgb,var(--tf-warning)_8%,transparent)] text-[var(--tf-warning)]",
    danger:
      "border-[color-mix(in_srgb,var(--tf-danger)_28%,transparent)] bg-[color-mix(in_srgb,var(--tf-danger)_8%,transparent)] text-[var(--tf-danger)]",
...
export function TfMetric({
  ...
  return (
    <div className="min-w-0 border-l border-[var(--tf-line)] pl-4 first:border-l-0 first:pl-0">
      <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--tf-text-subtle)]">
        {label}
      </div>
      <div className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-[var(--tf-text)]">
        {value}
      </div>
...
export function TfSignalDot({ active = true }: { active?: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
        "inline-block size-1.5 rounded-full bg-[var(--tf-signal)]",
        active && "tf-signal-pulse",
      )}
    />
  );
}
