    ...
    <main className="min-h-screen bg-[var(--tf-canvas)] text-[var(--tf-text)]">
      ...
      <header className="relative overflow-hidden border-b border-[var(--tf-line)]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-16 pt-20 lg:px-8 lg:pb-24 lg:pt-28">
          <Link href="/" className="mb-12 inline-flex items-center gap-2 text-xs text-[var(--tf-text-subtle)] transition hover:text-[var(--tf-text)]"><ArrowLeft className="size-3.5" /> Home</Link>
          <TfLabel>{eyebrow}</TfLabel>
          <h1 className="mt-4 max-w-4xl text-balance text-4xl font-semibold tracking-[-0.04em] sm:text-6xl">{title}</h1>
          <p className="mt-6 max-w-2xl text-base leading-8 text-[var(--tf-text-muted)] sm:text-lg">{description}</p>
        </div>
      </header>
