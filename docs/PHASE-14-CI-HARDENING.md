...
CI also disables telemetry, bounds npm registry fetch behavior, and retains workflow-level concurrency cancellation. The security dependency-audit job installs **production dependencies only** with lifecycle scripts disabled because its scope is production dependency auditing; the application CI build still installs normal lifecycle scripts so the production build remains representative.
...
- Dependency-audit installs are production-only, bounded by a 60-second fetch timeout and two fetch retries.
