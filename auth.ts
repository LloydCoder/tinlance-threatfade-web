...
const ThreatFadeOIDC: OAuthConfig<ThreatFadeProfile> = {
  ...
  profile(profile: ThreatFadeProfile) {
    return {
      id: profile.sub,
...
async function refreshAccessToken(refreshToken: string) {
  ...
  try {
    ...
    return {
      ...
      expires_at: Date.now() + Math.max(60, Number(payload.expires_in ?? 3600) - 60) * 1000,
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, account, profile }) {
      if (account?.access_token) {
        ...
        token.access_token_expires_at =
          (account.expires_at ?? Math.floor(Date.now() / 1000) + 3600) * 1000 - 60_000;
        token.provider = account.provider;
        if (!token.tf_session) {
          token.tf_session = (
            await engineIdentityRequest<{ session_token: string }>(
        ...
        typeof token.access_token_expires_at === "number" &&
        Date.now() >= token.access_token_expires_at
      ) {
        try {
          const refreshed = await refreshAccessToken(token.refresh_token);
          token.access_token = refreshed.access_token;
          token.access_token_expires_at = refreshed.expires_at;
          token.refresh_token = refreshed.refresh_token;
        } catch {
          token.auth_error = "RefreshAccessTokenError";
        }
      }
    ...
    async session({ session, token }) {
      if (session.user) session.user.id = typeof token.sub === "string" ? token.sub : "";
      return session;
    },
  },
  events: {
    async signOut({ token }) {
      if (
        !token?.access_token ||
        typeof token.access_token !== "string" ||
        typeof token.tf_session !== "string"
      )
        return;
      try {
        ...
        /* local cookie invalidation remains authoritative */
      }
    },
  },
};
