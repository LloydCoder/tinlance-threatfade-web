...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const docs = docsSections.map((doc) => ({ url: `${siteConfig.url}/docs/${doc.slug}`, lastModified: "2026-08-23", changeFrequency: "monthly" as const, priority: 0.8 }));
  return [...routes.map((path) => ({ url: `${siteConfig.url}${path}`, lastModified: new Date(), changeFrequency: path === "/" ? "weekly" as const : "monthly" as const, priority: path === "/" ? 1 : 0.7 })), ...docs, ...research];
}
