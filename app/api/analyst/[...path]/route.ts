...
const QUERY_KEYS = new Set(["status", "limit", "offset", "sort", "order"]);
...
function authorizationHeader(request: NextRequest) {
  ...
  if (process.env.THREATFADE_SOC_SERVICE_MODE === "true") {
    ...
    if (token && token.length <= MAX_TOKEN) return `Bearer ${token}`;
  }
  return null;
}
...
async function forward(request: NextRequest, path: string[]) {
  ...
  try {
    ...
    const status = error instanceof DOMException && error.name === "AbortError" ? 504 : 503;
    return NextResponse.json({ error: status === 504 ? "Analyst service timed out" : "Analyst service unavailable" }, { status });
  } finally {
    clearTimeout(timer);
  }
}
