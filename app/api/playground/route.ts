...
function rate(request: Request) {
  ...
  if (!old || old.resetAt <= now) {
    hits.set(k, { count: 1, resetAt: now + WINDOW_MS });
    return { blocked: false, remaining: 19, resetAt: now + WINDOW_MS };
  }
  if (old.count >= MAX_REQUESTS) return { blocked: true, remaining: 0, resetAt: old.resetAt };
  old.count += 1;
  return { blocked: false, remaining: MAX_REQUESTS - old.count, resetAt: old.resetAt };
}
export async function GET() {
  return NextResponse.json({
    mode: "curated-only",
    arbitrary_uploads: false,
...
export async function POST(request: Request) {
  ...
  if (length > 2048)
    return NextResponse.json(
      { error: "Request body exceeds the 2 KB playground limit." },
      { status: 413 },
    );
  try {
    const parsed = playgroundRequestSchema.safeParse(await request.json());
    if (!parsed.success)
      return NextResponse.json({ error: "Invalid playground scenario." }, { status: 400 });
    const response = NextResponse.json(getPlaygroundDataset(parsed.data.scenario));
    response.headers.set("Cache-Control", "no-store");
    response.headers.set("X-Playground-Mode", "curated-only");
