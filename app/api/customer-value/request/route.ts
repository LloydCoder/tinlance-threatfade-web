  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [to],
        reply_to: email,
        subject: `ThreatFade customer ${parsed.data.type} request`,
        html,
      }),
      redirect: "error",
      cache: "no-store",
    ...
    return NextResponse.json(
      { error: "Customer request service temporarily unavailable" },
      { status: 503 },
    );
  }
  try {
    await trackServerEvent("customer_request", request);
  } catch {
