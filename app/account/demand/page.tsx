    ...
    <main className="mx-auto max-w-5xl px-6 py-12">
      ...
      <section className="mt-8 grid gap-5 rounded-2xl border border-[var(--tf-border)] p-6 md:grid-cols-2">
        ...
        <button
          type="button"
          onClick={() => void score()}
          className="rounded-lg bg-slate-950 px-5 py-3 text-sm font-semibold text-white md:col-span-2"
        >
          Calculate explainable scores
        </button>
      </section>
      {error && (
        <p role="alert" className="mt-5 rounded-lg border border-red-300 p-4 text-sm">
          {error}
        </p>
      )}
      {result && (
        <section className="mt-6 grid gap-5 md:grid-cols-2">
          <div className="rounded-2xl border p-6">
            <p className="text-sm text-[var(--tf-text-muted)]">ThreatFade fit</p>
            <p className="mt-2 text-5xl font-semibold">
              {result.threatFadeFit}
              <span className="text-lg">/100</span>
            </p>
          </div>
          <div className="rounded-2xl border p-6">
            <p className="text-sm text-[var(--tf-text-muted)]">Buying intent</p>
            <p className="mt-2 text-5xl font-semibold">
              {result.buyingIntent}
              <span className="text-lg">/100</span>
            </p>
