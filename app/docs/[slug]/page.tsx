...
export default async function DocPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { content } = await compileMDX({
    source: doc.source,
    options: { parseFrontmatter: true, blockJS: true, blockDangerousJS: true },
    ...
    <article className="min-w-0">
      ...
      <header className="border-b border-[var(--tf-line)] pb-8">
        <div className="flex flex-wrap items-center gap-3">
          <TfLabel>{doc.frontmatter.category}</TfLabel>
          <TfBadge tone="neutral">{doc.frontmatter.version}</TfBadge>
          <TfBadge tone={doc.frontmatter.status === "implemented" ? "signal" : "warning"}>
        ...
        <p className="mt-4 max-w-3xl text-base leading-8 text-[var(--tf-text-muted)]">
          {doc.frontmatter.description}
        </p>
        <time
          dateTime={doc.frontmatter.updated}
          className="mt-5 block font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--tf-text-subtle)]"
        >
          Updated {doc.frontmatter.updated}
        </time>
      </header>
