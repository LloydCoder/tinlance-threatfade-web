  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      <div className="flex flex-wrap items-end justify-between gap-6">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-[var(--tf-text-muted)]">
            Triage evidence-backed detections, investigate provenance, record disposition, and
            preserve the analyst trail.
          </p>
        </div>
        <Link
          href="/correlation"
      ...
      <section
        ...
        className="mt-8 grid gap-3 md:grid-cols-[1fr_180px_180px_auto]"
      >
      ...
      {error && (
        <div role="alert" className="mt-5 rounded-lg border border-red-400/40 px-4 py-3 text-sm">
          {error}
        </div>
      )}
        ...
        <table className="w-full min-w-[980px] text-left text-sm">
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-[var(--tf-text-muted)]">
                  {error ? "No data available." : "No detections match the current filter."}
                </td>
              </tr>
            )}
          </tbody>
      ...
      <nav
        aria-label="Detection pagination"
        className="mt-5 flex items-center justify-between gap-4 text-sm"
      >
        <span className="text-[var(--tf-text-muted)]">
          {data.pagination.total} total · showing{" "}
          {data.pagination.total === 0 ? 0 : data.pagination.offset + 1}–
          {Math.min(data.pagination.offset + data.items.length, data.pagination.total)}
        </span>
