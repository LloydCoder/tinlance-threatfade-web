...
export default function InvestigationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => { if (!id) return; fetch(`/api/analyst/detections/${encodeURIComponent(id)}`, { cache: "no-store" }).then(async (r) => { if (!r.ok) throw new Error(`Detection unavailable (${r.status})`); return r.json(); }).then(setData).catch((e) => setError(e instanceof Error ? e.message : "Detection unavailable")); }, [id]);

  async function mutate(path: string, method: string, body: unknown) {
    setBusy(true); setError(null);
    try { const r = await fetch(`/api/analyst/${path}`, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); if (!r.ok) throw new Error(`Request failed (${r.status})`); const fresh = await fetch(`/api/analyst/detections/${id}`, { cache: "no-store" }); setData(await fresh.json()); }
  ...
  return <main className="mx-auto max-w-7xl px-6 py-10">
    ...
    <header className="mt-6 flex flex-wrap items-start justify-between gap-6"><div><p className="text-xs uppercase tracking-[0.18em] text-[var(--tf-text-muted)]">Investigation #{d.id}</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.04em]">{d.subject}</h1><p className="mt-2 text-[var(--tf-text-muted)]">{d.source} · {d.mitre_ttp} · {new Date(d.created_at).toLocaleString()}</p></div><div className="flex gap-2"><button disabled={busy} onClick={() => void mutate(`detections/${id}/workflow`, "PATCH", { status: "investigating" })} className="rounded-lg border px-4 py-2 text-sm">Start investigation</button><button disabled={busy} onClick={() => void mutate(`detections/${id}/workflow`, "PATCH", { status: "contained" })} className="rounded-lg border px-4 py-2 text-sm">Mark contained</button></div></header>
    {error && <p role="alert" className="mt-4 rounded-lg border border-red-400/40 px-4 py-3 text-sm">{error}</p>}
    <div className="mt-8 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
      <section className="space-y-6">
        <div className="rounded-xl border border-[var(--tf-border)] p-6"><h2 className="font-semibold">Evidence</h2><p className="mt-2 text-sm text-[var(--tf-text-muted)]">Evidence is shown with provenance metadata; confidence is not evidence.</p><div className="mt-5 space-y-3">{data.evidence.length ? data.evidence.map((e: any) => <div key={e.id} className="rounded-lg border border-[var(--tf-border)] p-4 text-sm"><div className="flex justify-between gap-4"><span className="font-medium">{e.type}</span><span>{e.media_type}</span></div><div className="mt-2 break-all font-mono text-xs text-[var(--tf-text-muted)]">{e.hash}</div><div className="mt-2 text-xs text-[var(--tf-text-muted)]">Collected {new Date(e.collected_at).toLocaleString()} · {e.size_bytes} bytes</div></div>) : <p className="text-sm text-[var(--tf-text-muted)]">No linked evidence records.</p>}</div></div>
        <div className="rounded-xl border border-[var(--tf-border)] p-6"><h2 className="font-semibold">Disposition</h2><textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={4000} placeholder="Analyst note / rationale" className="mt-4 min-h-24 w-full rounded-lg border border-[var(--tf-border)] bg-transparent p-3 text-sm" /><div className="mt-3 flex flex-wrap gap-2">{["true_positive", "false_positive", "benign", "duplicate", "insufficient_evidence", "needs_tuning"].map((reason) => <button key={reason} disabled={busy} onClick={() => void mutate(`detections/${id}/disposition`, "POST", { reason, note })} className="rounded-lg border px-3 py-2 text-xs">{reason.replaceAll("_", " ")}</button>)}</div></div>
      </section>
