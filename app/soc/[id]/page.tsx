...
export default function InvestigationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/analyst/detections/${encodeURIComponent(id)}`, { cache: "no-store" })
      .then(async (r) => {
        if (!r.ok) throw new Error(`Detection unavailable (${r.status})`);
        return r.json();
      })
      .then(setData)
  ...
  async function mutate(path: string, method: string, body: unknown) {
    ...
    setError(null);
    try {
      const r = await fetch(`/api/analyst/${path}`, {
        method,
      ...
      <header className="mt-6 flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-[var(--tf-text-muted)]">
            {d.source} · {d.mitre_ttp} · {new Date(d.created_at).toLocaleString()}
          </p>
        </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <section className="space-y-6">
          <div className="rounded-xl border border-[var(--tf-border)] p-6">
            ...
            <p className="mt-2 text-sm text-[var(--tf-text-muted)]">
              Evidence is shown with provenance metadata; confidence is not evidence.
            </p>
                ...
                data.evidence.map((e: any) => (
                  <div key={e.id} className="rounded-lg border border-[var(--tf-border)] p-4 text-sm">
                    ...
                    <div className="mt-2 text-xs text-[var(--tf-text-muted)]">
                      Collected {new Date(e.collected_at).toLocaleString()} · {e.size_bytes} bytes
                    </div>
                  </div>
                ))
              ) : (
        ...
        <aside className="space-y-6">
              ...
              {data.dispositions.map((x: any) => (
                <div key={x.id} className="border-l-2 pl-3">
                  ...
                  <div className="text-xs text-[var(--tf-text-muted)]">
                    {x.analyst} · {new Date(x.created_at).toLocaleString()}
                  </div>
          ...
          <div className="rounded-xl border border-[var(--tf-border)] p-6">
            ...
            <p className="mt-2 text-sm text-[var(--tf-text-muted)]">
              Use the evidence timeline to distinguish observed events from analyst conclusions.
            </p>
