...
const empty = { detectionCount: 0, investigationCount: 0, dispositionCount: 0, activeMembers: 1, environmentCount: 1, integrationRequestCount: 0, customDetectionRequestCount: 0 };
...
describe("customer value lifecycle", () => {
  ...
  it("marks core activation milestones from observed activity", () => {
    const snapshot = { ...empty, detectionCount: 2, investigationCount: 1, dispositionCount: 1, activeMembers: 2 };
  ...
  it("creates explainable expansion signals without automatic upgrades", () => {
    const signals = getExpansionSignals({ ...empty, detectionCount: 25, activeMembers: 3, environmentCount: 2, integrationRequestCount: 1, customDetectionRequestCount: 1 });
