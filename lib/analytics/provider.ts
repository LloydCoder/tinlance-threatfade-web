...
function posthogConfigured() {
  return Boolean(process.env.POSTHOG_PROJECT_API_KEY && process.env.POSTHOG_PROJECT_ID);
}
...
class PostHogProvider implements AnalyticsProvider {
  async capture(event: AnalyticsEvent, context: AnalyticsContext) {
    ...
    const response = await fetch(`${posthogHost()}/capture/`, {
      method: "POST",
      headers: { Authorization: `Bearer ${process.env.POSTHOG_PROJECT_API_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({ api_key: process.env.POSTHOG_PROJECT_API_KEY, event: event.name, distinct_id: context.distinctId, properties, timestamp: context.receivedAt }),
      cache: "no-store",
      redirect: "error",
  ...
  async funnel(days: number) {
    if (!posthogConfigured() || !process.env.POSTHOG_API_KEY) return { configured: false, metrics: [] };
    ...
    const response = await fetch(`${posthogHost()}/api/projects/${encodeURIComponent(process.env.POSTHOG_PROJECT_ID)}/query/`, {
      method: "POST",
