...
describe("ThreatFade analytics taxonomy", () => {
  ...
  it("rejects unknown events and oversized paths", () => {
    expect(analyticsEventSchema.safeParse({ name: "made_up_event", path: "/" }).success).toBe(false);
    expect(analyticsEventSchema.safeParse({ name: "page_view", path: "x".repeat(513) }).success).toBe(false);
  });

  it("allows only scalar event values", () => {
    expect(analyticsEventSchema.safeParse({ name: "page_view", path: "/", value: { sensitive: { nested: true } } }).success).toBe(false);
  });
});
